...
pub fn nixos_dir() -> PathBuf {
    std::env::var("LEECH_NIXOS_DIR").map_or_else(|_| home().join("nixos"), PathBuf::from)
}
...
pub fn compose_file() -> PathBuf {
    container_dir().join("docker-compose.leech.yml")
}
...
pub fn resolve_dir(dir: Option<&str>) -> Result<PathBuf> {
    ...
    path.canonicalize()
        .map_err(|_| LeechError::DirNotFound(path.display().to_string()))
}
...
pub fn proj_name(slug: &str, instance: Option<&str>) -> String {
    match instance {
        Some(i) if i != "1" && !i.is_empty() => format!("leech-{slug}-{i}"),
        _ => format!("leech-{slug}"),
    }
}
...
mod tests {
    ...
    #[test]
    fn name_no_instance() {
        assert_eq!(proj_name("projects", None), "leech-projects");
    }

    #[test]
    fn name_instance_2() {
        assert_eq!(proj_name("projects", Some("2")), "leech-projects-2");
    }

    #[test]
    fn name_instance_1_ignored() {
        assert_eq!(proj_name("projects", Some("1")), "leech-projects");
    }

    #[test]
    fn name_instance_empty_ignored() {
        assert_eq!(proj_name("projects", Some("")), "leech-projects");
    }
