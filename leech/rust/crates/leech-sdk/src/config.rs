//! `LeechConfig` — loads and parses `~/.leech` (KEY=value, bash-sourceable format).
...
/// Parsed ~/.leech config (KEY=value format, sourceable by bash).
...
impl LeechConfig {
    /// Load config from ~/.leech (or $LEECH_CONFIG).
    pub fn load() -> Result<Self> {
        ...
        if path.exists() {
            let content = fs::read_to_string(&path)
                .map_err(|e| LeechError::Config(format!("reading {}: {e}", path.display())))?;
...
fn config_path() -> PathBuf {
    std::env::var("LEECH_CONFIG").map_or_else(|_| dirs_home().join(".leech"), PathBuf::from)
}
