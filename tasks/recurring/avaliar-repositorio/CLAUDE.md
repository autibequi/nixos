...
2. Escaneie arquivos modificados recentemente (`find . -name '*.nix' -newer .ephemeral/notes/avaliar-repositorio/contexto.md 2>/dev/null || find . -name '*.nix' -mtime -1`)
3. Procure por:
...
5. Se encontrar algo actionable, crie uma task em `tasks/pending/` com CLAUDE.md descrevendo o que fazer
...
- NÃO crie tasks duplicadas (verifique pending/ e recurring/ antes)
