...
tasks/               # Autonomous task system
  recurring/         # Immortal tasks (run hourly, return to queue)
    usage-tracker/   # API usage monitoring (haiku, always)
    doctor/          # System health checks (haiku, always)
    radar/           # Jira/Notion monitoring (haiku, always, MCP)
    avaliar-claudinho/  # Work project analysis (sonnet, night)
    avaliar-repositorio/ # NixOS config analysis (sonnet, night)
    evolucao/        # Self-improvement meta-agent (sonnet, night)
...
vault/               # Obsidian vault (dashboard + communication)
  ...
  tasks → ../tasks   # Symlink
  reports → ../.ephemeral/notes  # Symlink
