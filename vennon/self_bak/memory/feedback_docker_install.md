...
description: Lições do comando vennon install sobre SSH, TTY e go mod download
type: feedback
---

**Regras aprendidas no desenvolvimento do `vennon install`:**
...
5. **`install` acessa credenciais — `run`/workers nunca**
   ...
   - `vennon install` — acessa `~/.ssh`, `~/.npmrc`, tokens. Gera artefatos no projeto (vendor/, node_modules/).
   - `vennon run` / Dockerfile / workers — build e runtime limpos, sem credenciais. Consomem os artefatos gerados pelo install.
   Para Node.js: o Dockerfile não faz `npm install`. Só sobe o servidor. O bind mount expõe o `node_modules/` gerado pelo install.

**Why:** Erros encontrados ao implementar `vennon install monolito` em 2026-03-18. Separação de credenciais exigida pelo usuário em 2026-03-18 ao dockerizar bo-container.
