...
_do_status_render() {
  ...
  local _term_h="${LINES:-$(tput lines 2>/dev/null || echo 24)}"
...
_render_frame() {
  _do_status_render > "$_tmpfile" 2>/dev/null
...
# Entrar em alternate screen e esconder cursor imediatamente
      ...
      if [[ "$_key" == $'\033' ]]; then
        ...
        case "$_seq" in
          ...
          '[B')  # Down arrow
            ...
            break
            ;;
          '[M')  # X10 mouse event — parseia scroll, drena o resto
            local _btn="" _mx="" _my=""
            read -t 0.05 -rsn1 _btn 2>/dev/null || true
            read -t 0.05 -rsn1 _mx  2>/dev/null || true
            read -t 0.05 -rsn1 _my  2>/dev/null || true
            # Scroll up = byte 96 (0x60), scroll down = byte 97 (0x61)
            if [[ "$_btn" == $'\x60' ]]; then
              _cursor_idx=$(( (_cursor_idx - 1 + ${#_svc_list[@]}) % ${#_svc_list[@]} ))
              _cursor_svc="${_svc_list[$_cursor_idx]}"
              break
            ...
            ;;
          '[<')  # SGR mouse event — parseia scroll, drena o resto
            local _sgr_data=""
            local _c
            ...
            # 64;<x>;<y>M = scroll up, 65;<x>;<y>M = scroll down
            if [[ "$_sgr_data" == 64\;*M ]]; then
              _cursor_idx=$(( (_cursor_idx - 1 + ${#_svc_list[@]}) % ${#_svc_list[@]} ))
              _cursor_svc="${_svc_list[$_cursor_idx]}"
              break
            elif [[ "$_sgr_data" == 65\;*M ]]; then
              _cursor_idx=$(( (_cursor_idx + 1) % ${#_svc_list[@]} ))
              _cursor_svc="${_svc_list[$_cursor_idx]}"
              break
            fi
