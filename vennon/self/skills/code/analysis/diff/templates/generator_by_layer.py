"""
code/analysis/diff — Variante por camada.

Agrupa os arquivos do diff em seções por tipo (Handlers, Services, Repositories,
Workers, etc.), cada seção com sua própria árvore interativa colapsável.
...
Uso:
    ...
    2. Configurar REPO, BRANCH, DIFF_FILE, OUTPUT_FILE, LAYERS abaixo.

    3. Preencher DESCRIPTIONS (opcional) — mesma estrutura do generator.py.

    4. Rodar:
           python3 generator_by_layer.py > /dev/null

    5. Abrir no Chrome:
           python3 /tmp/open_diff.py   # ver SKILL.md para o script completo

Customizar LAYERS para o repo alvo:
    Monolito  → handlers/, services/, repositories/, workers/, migration/, mocks/
    BO        → pages/, components/, services/, router/
    Front     → pages/, containers/, components/, composables/, services/, types/
"""
...
OUTPUT_FILE = '/tmp/mono_diff_bylayer.html'
...
# Camadas — ordem importa: o arquivo entra na PRIMEIRA camada que der match.
# ('Label', cor_hex, fn(path) -> bool)
# A última entrada deve ser catch-all (lambda p: True) para "Outros".
LAYERS = [
...
DESCRIPTIONS = {
    ...
    #      'retorna estrutura flat sem dados pesados'),
}
...
# ── Classificar arquivos por camada ──────────────────────────────────────────
...
with open(DIFF_FILE) as f:
    for line in f:
        ...
        if len(parts) == 2:
            all_files.append((parts[0], parts[1]))

# Distribuir cada arquivo para a primeira camada que der match
buckets = {label: [] for label, _, _ in LAYERS}
for status, path in all_files:
    for label, color, matcher in LAYERS:
        if matcher(path):
            buckets[label].append((status, path))
            break

total_a  = sum(1 for s, _ in all_files if s == 'A')
total_m  = sum(1 for s, _ in all_files if s == 'M')
total_d  = sum(1 for s, _ in all_files if s == 'D')
total_ch = sum(1 for s, p in all_files if is_chapter(p))

# ── Gerar seções HTML ─────────────────────────────────────────────────────────

layer_colors = {label: color for label, color, _ in LAYERS}

sections_html = ''
for label, color, _ in LAYERS:
    files = buckets[label]
    if not files:
        continue
...
<div class="section">
  <div class="section-header" onclick="toggleSection(this)">
    ...
    <span class="section-stats">{stats}</span>
  </div>
  <div class="section-body">
    <div class="tree-container" id="tree-{label.lower()}">
{tree_html}
    </div>
  </div>
</div>
...
<head>
<meta charset="UTF-8">
<title>Diff por Camada — {REPO} — {BRANCH}</title>
<style>
:root {{
...
body {{
  ...
  font-size: 13px; min-height: 100vh; padding-bottom: 6rem;
}}

/* ── Header ── */
.header {{
  background: var(--ctp-mantle); border-bottom: 2px solid var(--ctp-surface1);
  padding: 0.9rem 2rem; display: flex; align-items: center; gap: 1.2rem;
...
.toggle-ann-btn {{
  ...
  color: var(--ctp-overlay0); font-size: 0.75rem; font-family: inherit;
  cursor: pointer; transition: all 0.15s; user-select: none;
}}
.toggle-ann-btn:hover {{ background: var(--ctp-surface1); color: var(--ctp-text); }}
.toggle-ann-btn.active {{ background: rgba(148,226,213,0.12); border-color: var(--ctp-teal); color: var(--ctp-teal); }}
body.hide-annotations .desc-tag {{ display: none; }}
...
/* ── Sections ── */
.sections {{ padding: 1rem 1.5rem; display: flex; flex-direction: column; gap: 0.5rem; }}
...
.section-arrow {{
  ...
  transition: transform 0.18s ease; width: 1em; flex-shrink: 0;
}}
.section.collapsed .section-arrow {{ transform: rotate(-90deg); }}
.section-label {{ font-weight: bold; font-size: 0.85rem; letter-spacing: 0.03em; }}
.section-count {{ color: var(--ctp-overlay0); font-size: 0.8rem; }}
.section-stats {{ display: flex; gap: 0.4rem; font-size: 0.78rem; margin-left: 0.3rem; }}
...
/* ── Tree ── */
.tree-container {{ padding: 0.8rem 1rem 0.8rem 1.4rem; line-height: 2; user-select: none; }}
.subtree {{
...
.legend {{
  ...
  margin-top: 0.5rem; font-size: 0.78rem; color: var(--ctp-overlay0);
}}
...
<div class="header">
  <span class="header-title">DIFF por Camada</span>
  <span class="repo-badge">{REPO}</span>
  <span class="branch-badge">{BRANCH}</span>
  <div class="stats">
    <span class="stat" style="color:var(--ctp-text)">{len(all_files)} files</span>
  ...
  <button class="toggle-ann-btn active" id="toggle-ann" onclick="toggleAnnotations()">// desc ON</button>
</div>
...
<div class="legend">
  ...
  <div class="leg" style="margin-left:auto; color:var(--ctp-overlay0)">clique seção = colapsar · clique arquivo = trilha</div>
</div>
...
function selectFile(fileEl) {{
  ...
  // Glow ancestors dentro da section-body (para a subtree correta)
  let node = fileEl.parentElement;
  while (node && !node.classList.contains('tree-container')) {{
...
function toggleAnnotations() {{
  const hidden = document.body.classList.toggle('hide-annotations');
  const btn = document.getElementById('toggle-ann');
  btn.textContent = hidden ? '// desc OFF' : '// desc ON';
