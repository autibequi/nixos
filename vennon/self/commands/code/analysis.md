...
```
/code:analysis diff           → árvore interativa de arquivos no Chrome
/code:analysis objects        → objetos modificados por camada
/code:analysis flows          → diagrama de fluxo no Chrome
/code:analysis report         → relatório consolidado da branch
/code:analysis inspect        → inspeção leve de qualidade
/code:analysis review         → review completo de PR (JIRA + escopo + fluxo + validação + veredito)
/code:analysis peer-reviews   → simula review dos 5 peers do monolito
/code:analysis gh-review      → review de PR via GitHub API (sem branch local)
/code:analysis github-evaluate → avaliar dev por histórico de PRs no GitHub
/code:analysis tdd           → TDD cycle (RED-GREEN-REFACTOR) no arquivo/função atual
/code:analysis debug         → debugging sistemático em 4 fases (reproduzir, hipóteses, isolar, verificar)
/code:analysis test          → plano de testes a partir do diff (cenários Happy/Sad/Weird com prioridade)
/code:analysis flutter       → knowledge-base do app doings (Dart/Flutter)
/code:analysis practices     → boas práticas — checklist automático no código atual
/code:analysis               → sem argumento: mostra este menu
```
