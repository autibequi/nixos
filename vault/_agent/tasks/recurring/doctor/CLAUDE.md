...
- Diretórios: vault/_agent/tasks/{recurring,pending,running,done,failed}, .ephemeral/{notes,usage,scratch}, scripts/
...
### Sistema de Tarefas
- vault/_agent/tasks/running/ está vazio (nada preso)
- vault/_agent/tasks/recurring/ tem as imortais
- Contextos em vault/_agent/tasks/*/contexto.md estão saudáveis (não corrompidos, não enormes)
...
- Se >90%, audit `du -sh /workspace/projetos/*/` pra identificar submódulos gigantes
- Tamanho do .ephemeral/ (não crescendo demais?)
- Tempo das últimas execuções de tasks (ficando mais lentas?)
- Locks em running/: verificar timestamp, diferenciar "ativo" (worker rodando) vs. "travado" (timeout)
