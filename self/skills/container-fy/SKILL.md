...
- Levantar / parar / debugar servicos existentes (`zion docker run/stop/logs/...`)
...
### 2. Gerar `zion/containers/<service>/`
...
Editar `zion/cli/src/lib/docker_services.sh`:
- `zion_docker_service_dir()` — case com var de `~/.zion`
- `zion_docker_known_services()` — adicionar na lista
...
```bash
zion docker run <service> [--env=sand] [--detach] [--debug]
...
zion docker flush <service>      # remove volumes anonimos
zion docker install <service>    # go work vendor / npm install
```
...
- npm install durante `docker compose build` (sem `zion docker install`)
...
- Logs do servidor → `zion docker logs`, nao no Debug Console
...
- Network: `nixos_default` (external) para comunicacao entre containers Zion
