...
}:
{
  ...
  nix.settings = {
    substituters = [
      ...
      # llm-agents.nix (numtide) — cache distinto do numtide.cachix.org
  ...
  # AppImage Support
  # binfmt desabilitado: systemd-binfmt.service levava ~1s no boot.
  # AppImages continuam a funcionar via appimage-run (wrapper manual).
  programs.appimage = {
    enable = true;
    binfmt = false;
  };
