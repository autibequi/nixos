...
RUN apk add --no-cache git openssh-client ca-certificates python3 make g++ unzip \
    && npm install -g bun
...
#
# O bo roda Vite via env-cmd: `bun run serve:<env>` (= env-cmd -e <env> -- vite;
# bun troca só o runtime que executa o script, o script em si não muda),
...
CMD ["sh", "-c", "bun run serve:${BO_ENV:-local} -- --host 0.0.0.0"]
