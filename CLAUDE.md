...
**Como reconhecer que está em `zion edit` ou `zion leech ~/nixos`:** se o seu CWD é a raiz deste repo (onde está `CLAUDE.md`, `flake.nix`, `zion/`) **e** existe o path `/workspace/logs` (ex.: journal do host), você está numa sessão **`zion edit`** ou **`zion leech ~/nixos`** — assuma a identidade Zion e use este documento como contexto.

Assim, ao entrar (sobretudo em `zion edit` ou `zion leech ~/nixos`), você já sabe: **este projeto = NixOS + Zion** e **você = Zion, gestor do repo do usuário e do seu próprio ambiente de agentes**.

### 1.1 Primeiros passos ao carregar (checklist)

Ao abrir o projeto, faça em segundos:

1. **Confirmar o modo:** CWD é a raiz deste repo? Existe `/workspace/logs`? → Se sim, você está em **`zion edit`** ou **`zion leech ~/nixos`** (este repo = `/workspace/mnt`). Se o CWD for outro projeto, você está em **run/shell/leech** (projeto do usuário em `/workspace/mnt`).
...
| **`zion edit`** | Abre sessão com **~/nixos** em `/workspace/mnt` e `/workspace/logs` (único modo com mount de logs). Project name fixo `zion-projects`. Equivalente a `zion leech ~/nixos`. |
| **`zion leech [dir]`** | Sessão efêmera em qualquer pasta. Auto-detecta nixos repo (flake.nix + configuration.nix) → monta logs automaticamente. Alias: `l`. |
...
| **`zion leech ~/nixos`** | ❌ (mnt = nixos) | ✅ (journal ro, auto-detect) | **~/nixos** (este repo) |
...
- O Compose define **dois serviços**: (1) **leech** (`docker-compose.zion.yml`) — sessão interativa efêmera (Cursor/Claude/OpenCode); (2) **puppy** (`docker-compose.puppy.yml`) — container persistente com daemon interno (scheduler) + runner de tasks. Ambos usam os mesmos **`x-base-volumes`**.
...
- **`x-base-volumes`** — usado por **leech** (sessão interativa) e **puppy** (worker persistente).
  - **`/zion`** ← pasta `zion/` do repo (sempre a mesma).
  - **`/workspace/obsidian`** ← vault Obsidian (host: `OBSIDIAN_PATH`).
  - **`/workspace/mnt`** ← projeto do usuário (host: `CLAUDIO_MOUNT`, ex.: `~/projects`). **working_dir** do leech é `/workspace/mnt`.
...
- **`zion edit`** / **`zion leech ~/nixos`** — não são serviços separados; é o **mesmo serviço leech** rodado com parâmetros diferentes pelo CLI:
...
| **`zion leech ~/nixos`** | Repo NixOS (este repo, auto-detect) | ❌ (mnt = nixos) | ✅ (journal) |
| **`zion leech <outro-dir>`** | Dir especificado | ❌ | ❌ |
...
|---------|---------|-----|----------------------|
| **leech** | `docker-compose.zion.yml` | Sessão interativa efêmera (Cursor/Claude/OpenCode). Fica com `sleep infinity` até o CLI fazer `exec` com o engine. | Default: `sleep infinity`. O `zion leech`/`edit` faz `exec` no container com bash que roda bootstrap + engine. |
...
├── Justfile               ← Atalhos do host (switch, stow, install, gen, proxy, etc.).
...
- **Comandos principais:** `continue` (default), `new`, `resume`, `shell`, `start`, `edit`, `leech`, `puppy` (start/stop/run/status/logs/shell/tick), `logs`, `status`, `new-task`, `build`, `down`, `destroy`, `update`, `init`.
- **`edit`:** monta este repo em `/workspace/mnt` e ainda monta `/workspace/logs`; arquivo: `host_edit.sh`; project name `zion-projects`. Equivalente a `zion leech ~/nixos`.
- **`leech [dir]`:** sessão efêmera em qualquer pasta. Auto-detecta nixos repo (flake.nix + configuration.nix) → monta `/workspace/logs` automaticamente, usa project name `zion-projects`. Para outros dirs: project name derivado do slug; arquivo: `leech.sh`.
