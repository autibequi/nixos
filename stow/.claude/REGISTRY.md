...
| `claude-oauth-usage.sh` | bash | Uso do plano via OAuth token (`~/.claude/.credentials.json`). Sem browser, sem cookies. Endpoint: `api.anthropic.com/api/oauth/usage`. Modos: `--json` (bruto), `--waybar` (JSON waybar), `--short` (uma linha). Waybar: módulo `custom/claude-usage`. |
