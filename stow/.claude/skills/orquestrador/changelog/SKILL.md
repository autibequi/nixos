...
Após montar o changelog, **sempre** salvar o conteúdo completo em arquivo dentro de `obsidian/_agent/tasks/`, com o nome `changelog.<data>.md`, onde `<data>` é a data atual no formato `YYYY-MM-DD`.

**Onde salvar:**
- **Se invocado dentro de uma feature** (existe pasta `obsidian/_agent/tasks/FUK2-*/` ativa): salvar dentro da pasta da feature → `obsidian/_agent/tasks/<pasta>/changelog.<data>.md`
- **Se invocado standalone** (sem feature ativa): identificar a task a partir da branch atual de cada repositório e criar a pasta correspondente em `obsidian/_agent/tasks/`:
  ...
  3. Criar pasta `obsidian/_agent/tasks/<codigo>/` se não existir
  4. Salvar em `obsidian/_agent/tasks/<codigo>/changelog.<data>.md`
  5. Se não for possível extrair um código de task da branch (ex: branch `main`), perguntar ao dev qual código usar

Exemplo dentro de feature: `obsidian/_agent/tasks/FUK2-1234/changelog.2026-03-09.md`
Exemplo standalone: `obsidian/_agent/tasks/FUK2-1234/changelog.2026-03-09.md`
...
- **Sempre salvar o changelog em arquivo dentro de `obsidian/_agent/tasks/`** — na pasta da feature se houver, senão criar pasta a partir do código da branch
