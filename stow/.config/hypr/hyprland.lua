...
-- GDK_BACKEND: ver modules/system/base.nix (environment.sessionVariables) —
-- NIXOS_OZONE_WL, MOZ_ENABLE_WAYLAND e QT_QPA_PLATFORM já vivem lá.
...
-- Hardware + UI rules
require("rules.monitors")
require("rules.windowrules")
require("rules.generated-colors")

-- Keybinds e workspaces (populam o registry keymap)
require("keybinds.application")
require("keybinds.systemtools")
require("keybinds.workspace")
require("keybinds.special-workspaces")

-- Cheatsheet/help/picker — consomem o registry, vêm depois
require("ui.picker")
require("ui.hyprshortcuts")
require("ui.help")

-- Profiles + features de produtividade
require("productivity.profiles")     -- default/focus/meeting/battery cycle (SUPER+SHIFT+P)
require("productivity.auto_profile") -- AC change → switch profile automático
require("productivity.cycler")       -- MOD3+I cicla por class
require("productivity.hud")          -- SUPER+; peek de estado
require("productivity.pomodoro")     -- SUPER+SHIFT+T (depende de profiles)
require("productivity.submaps")      -- smart_save/close/reload
require("productivity.followme")     -- MOD3+F sync workspaces entre monitores
require("productivity.chicote")      -- MOD3+S modo chicote (overlay raylib)
require("productivity.layouts")      -- snapshots de janelas por workspace (CLI: hypr-layout)

-- Hooks reativos (consomem registries populados pelos requires acima)
require("hooks.events")
require("hooks.specials-feed") -- SIGRTMIN+11 → waybar refresh

-- Watchers / REPL (re-habilitados 2026-05-20: io.popen → io.open + /proc)
require("watchers.swallow")       -- ppid via /proc/<pid>/status
require("watchers.theme_watcher") -- marker read via io.open
require("watchers.repl")          -- timer 500ms, eval file-based
