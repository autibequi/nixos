...
case "${1:-list}" in
  list)
    fetch_all "/tasks" \
      | jq -c '[ .[] | {id, content, priority, due:(.due.string // ""), project_id, section_id} ]' \
      || echo '[]'
    ;;
  projects)
    fetch_all "/projects" \
  ...
  drop)
    ...
    ;;
  *)
    echo '{"error":"usage: list|projects|sections|inbox-id|menu-inbox|add <texto> [project_id]|done|drop"}'; exit 1
    ;;
esac
