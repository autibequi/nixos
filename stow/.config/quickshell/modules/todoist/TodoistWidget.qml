...
Scope {
    ...
    readonly property bool canDrag: root.scope !== "all" && root.scope !== "upcoming"

    onScopeChanged: root.sectionScope = "all"
    ...
    function displayItems() {
        ...
        for (var k = 0; k < noSec.length; k++) out.push(noSec[k])
        var secs = root.projectSections()
        for (var i = 0; i < secs.length; i++) {
            var inSec = rows.filter(function (t) { return t.section_id === secs[i].id })
            if (inSec.length === 0) continue
            out.push({ header: secs[i].name, count: inSec.length, secId: secs[i].id })
            for (var j = 0; j < inSec.length; j++) out.push(inSec[j])
        }
        return out
    }
    ...
    PanelWindow {
        ...
        Rectangle {
            ...
            radius: 14
            color: Qt.rgba(0.04, 0.055, 0.09, 0.95)   // semi-transparente (levemente opaco)
