...
- **OBSIDIAN_PATH** (opcional) — caminho do vault Obsidian; se não definir, usa `~/.ovault`. Necessário para montar `/obsidian` no container. O host (repo nixos) é sempre montado em `/host` via `$HOME/nixos`.
...
Com `engine=cursor`, o `claudio run` sobe o **Cursor CLI** (`agent`) no container, com o projeto em `/workspace`. Autenticação:
