...
**Why:** O container monta `zion/` em `/workspace/self`. Os arquivos em `scripts/` (raiz do repo) são apenas symlinks → `../zion/scripts/`. Editar `scripts/` diretamente não afeta o container. Erro cometido: editei scripts em `scripts/` (raiz) — o container continuou usando as versões antigas de `zion/scripts/`.
