# Le clock: dos agent.md e atualiza zion-agents.nix
zion_load_config

ZION_DIR="${ZION_NIXOS_DIR:-$HOME/nixos}/zion"
NIX_FILE="${ZION_NIXOS_DIR:-$HOME/nixos}/modules/zion-agents.nix"
...
for try in /workspace/mnt/modules/zion-agents.nix /workspace/nixos/modules/zion-agents.nix; do
...
if [ ! -f "$NIX_FILE" ]; then
  echo "zion-agents.nix nao encontrado"
  exit 1
fi
...
if [ ${#SCHEDULED[@]} -eq 0 ]; then
  echo "Nenhum agente com campo clock: encontrado."
  exit 0
fi

# Gerar bloco scheduled para o .nix
BLOCK="  # Agents com timer automatico: { name, onBootSec, onActiveSec }"$'\n'
BLOCK+="  # Gerado por \`zion agents cron\` -- edite clock: nos agent.md e rode o comando"$'\n'
BLOCK+="  scheduled = ["$'\n'
...
if ! diff -q "$NIX_FILE" "$TMPFILE" > /dev/null 2>&1; then
  cp "$TMPFILE" "$NIX_FILE"
  echo ""
  echo "zion-agents.nix atualizado."
