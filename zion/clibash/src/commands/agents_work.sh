# Executa todos os agent cards vencidos em agents/_schedule/
zion_load_config
...
if [ ! -d "$SCHEDULE" ]; then
  echo "[agents] agents/_schedule nao encontrado: $SCHEDULE"
  exit 1
fi
if [ ! -f "$RUNNER" ]; then
  echo "[agents] task-runner nao encontrado: $RUNNER"
  exit 1
fi
...
NOW=$(date +%s)
THRESHOLD=$((NOW + 300))  # 5min de tolerancia
DUE=()
...
if [ ${#DUE[@]} -eq 0 ]; then
  echo "[work] nenhum agent card vencido."
  exit 0
fi
...
# Executar em serie (evitar estouro de quota)
