# Executa um agente nomeado dentro do container Puppy, opcionalmente em uma task.
zion_load_config
...
if [[ -n "$task_name" ]]; then
  echo "[zion puppy] Rodando agente '$agent_name' na task '$task_name'..."
else
  echo "[zion puppy] Rodando agente '$agent_name'..."
fi
...
[[ -n "$task_name" ]] && env_flags+=(-e "TASK_NAME=$task_name")

# Build prompt
if [[ -n "$task_name" ]]; then
  base_prompt="Execute como agente $agent_name na task $task_name"
else
  base_prompt="Execute como agente $agent_name"
fi

if [[ -n "$headless" ]]; then
  deadline_str="$(date -u -d "+${timeout_secs} seconds" +%H:%M:%S 2>/dev/null || echo "${timeout_secs}s from now")"
  prompt="[HEADLESS MODE] Timeout: ${timeout_secs}s | Deadline: ${deadline_str} --- ${base_prompt}"
else
  prompt="$base_prompt"
fi
...
OBSIDIAN_PATH="$zion_obsidian_path" \
  ...
  bash -c "
    if [ -f '$agent_file' ]; then
      ...
      echo '[puppy_agent] Agent file not found: $agent_file' >&2
      exit 1
    fi
  "
