...
- **OBSIDIAN_PATH** (opcional) — caminho do vault Obsidian; se não definir, usa `~/.ovault`. Necessário para montar `/obsidian` no container. O host (repo nixos) é sempre montado em `/nixos` via `$HOME/nixos`.
