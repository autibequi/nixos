//! Agent commands — run and inspect background agent workers.

use anyhow::{bail, Result};
use zion_sdk::paths;

/// `zion agents run <name>` — run an agent immediately.
pub fn run(name: &str, steps: Option<u32>) -> Result<()> {
    let agent_file =
        paths::agent_file(name).ok_or_else(|| anyhow::anyhow!("agent '{name}' not found"))?;
    let runner = paths::task_runner().ok_or_else(|| anyhow::anyhow!("task-runner.sh not found"))?;
    let obsidian = paths::obsidian_path();
    let agents_dir = obsidian.join("agents");
    let schedule = agents_dir.join("_schedule");
    ...
    println!("[agents] '{name}' -> {card}  model={model}  steps={steps}");
    ...
    cmd.arg(&card)
        ...
        .env("TASK_CONTRACTORS_DIR", &agents_dir)
...
/// `zion agents work` — execute all due agent cards from schedule.
pub fn work(dry_run: bool) -> Result<()> {
    let schedule = paths::schedule_dir()
        .ok_or_else(|| anyhow::anyhow!("agents/_schedule not found"))?;
    ...
    if due.is_empty() {
        println!("[work] nenhum agent card vencido.");
        return Ok(());
    }
    ...
    let agents_dir = schedule.parent().map(|p| p.to_path_buf());
    let obsidian = paths::obsidian_path();
...
pub(crate) fn is_agent_card(content: &str) -> bool {
    let mut in_fm = false;
...
";
    ...
    #[test]
    fn is_agent_card_with_agent() {
        let content = "---\nmodel: haiku\nagent: wanderer\n---\nbody\n";
        assert!(is_agent_card(content));
    }

    #[test]
    fn is_agent_card_with_contractor_legacy() {
        let content = "---\ncontractor: coruja\nmodel: sonnet\n---\nbody\n";
        assert!(is_agent_card(content));
    }

    #[test]
    fn is_agent_card_without_agent() {
        let content = "---\nmodel: haiku\ntimeout: 900\n---\nbody\n";
        assert!(!is_agent_card(content));
    }
}
