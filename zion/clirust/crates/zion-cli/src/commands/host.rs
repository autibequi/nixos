...
pub fn stow(action: &str, reload: bool) -> Result<()> {
    ...
    match action {
        "status" | "st" => {
            println!("=== Stow status ===");
            crate::exec::run_in(&nixos, "stow", &["-d", "stow", "-t", &home, "-n", "-R", "."])
        }
        a @ ("restow" | "re" | "" | "delete" | "un" | "unstow") => {
            let flag = if a.starts_with('d') || a.starts_with('u') { "-D" } else { "-R" };
            println!("=== Stow {a} ===");
...
pub fn init(force: bool) -> Result<()> {
    ...
    if dest.exists() && !force {
        println!("{} already exists. Use --force to overwrite.", dest.display());
        return Ok(());
    }
    let example = paths::cli_dir().join("config.example");
    if example.exists() { std::fs::copy(&example, &dest)?; }
    else { std::fs::write(&dest, "# Zion config\nengine=claude\n# model=sonnet\n")?; }
...
pub fn set_engine(engine: &str) -> Result<()> {
    ...
    if !path.exists() { bail!("{} not found — run 'zion init' first", path.display()); }

    let content = std::fs::read_to_string(&path)?;
    let mut found = false;
    let new: Vec<String> = content.lines().map(|l| {
        if l.starts_with("engine=") { found = true; format!("engine={engine}") } else { l.to_string() }
    }).collect();
    let out = if found { new.join("\n") + "\n" } else { format!("{}\nengine={engine}\n", new.join("\n")) };
...
pub fn update() -> Result<()> {
    ...
    step(1, 4, "Gerando bash CLI", || {
        let s = Command::new("bashly").arg("generate")
            .current_dir(&cli)
            .env("LANG", "en_US.UTF-8").env("RUBYOPT", "-E utf-8")
            .output()?;
        if !s.status.success() { bail!("bashly generate failed:\n{}", String::from_utf8_lossy(&s.stderr)); }
        Ok(())
    })?;
    ...
    step(2, 4, "Instalando bash CLI", || {
        ...
        if old.is_symlink() { let _ = std::fs::remove_file(&old); }
    ...
    step(4, 4, "Building zion (rust)", || {
        if !clirust.exists() { return Ok(()); }
        let ok = Command::new("nix-shell")
            .args(["-p", "rustc", "cargo", "--run", "cargo build --release"])
            .current_dir(&clirust).output()
            .map(|o| o.status.success()).unwrap_or(false);
        if !ok {
            let s = Command::new("cargo").args(["build", "--release"]).current_dir(&clirust).output()?;
            if !s.status.success() { bail!("cargo build failed:\n{}", String::from_utf8_lossy(&s.stderr)); }
        }
...
fn step<F: FnOnce() -> Result<()>>(n: u32, total: u32, label: &str, f: F) -> Result<()> {
    print!("\r  \x1b[2m[{n}/{total}]\x1b[0m {label} \x1b[33m{}\x1b[0m", SPINNER[0]);
    let _ = std::io::stdout().flush();
    match f() {
        Ok(()) => { println!("\r  \x1b[2m[{n}/{total}]\x1b[0m {label} \x1b[32m✓\x1b[0m"); Ok(()) }
        Err(e) => { println!("\r  \x1b[2m[{n}/{total}]\x1b[0m {label} \x1b[31m✗\x1b[0m"); Err(e) }
    }
}
